...
export default () => {
	...
	step('SAP Fiori: Enter User Details', async (browser) => {
		...
		//Enter the password
		let input_password = By.xpath("//input[contains(@id, 'PASSWORD_FIELD-inner')]")
	...
	step('SAP Fiori: Sales Order Details - Entry', async (browser) => {
		...
		const page = (browser as any).page
		const frameSalesDocument = page.frames().find((frame) => frame.name().includes('itsframe1'))

		//Enter Sales Document Type
		const inputSDType = await frameSalesDocument.$('[title="Sales Document Type"]')
		await inputSDType.type('ZOR')
		//await browser.type(inputSDType, 'ZOR')

		//Sales Organization
		const inputSalesOrg = await frameSalesDocument.$('[title="Sales Organization"]')
		await inputSalesOrg.type('AU01')

		//Distribution Channel
		const inputDistributionChannel = await frameSalesDocument.$('[title="Distribution Channel"]')
		...
		//Sales Office
		const inputSalesOffice = await frameSalesDocument.$('[title="Sales Office"]')
	...
	step('SAP Fiori: Sales Order Details - Materials Line Entry', async (browser) => {
		//As the Sales Order objects are contained within an IFRAME - lets switch to it
		const page = (browser as any).page
		const frameSalesDocument = page.frames().find((frame) => frame.name().includes('itsframe1'))

		//Enter Sold-to Party
		const inputSoldToParty = await frameSalesDocument.$('[title="Sold-to party"]')
		await inputSoldToParty.type('1000000')
		await browser.sendKeys(Key.TAB)

		//Enter Ship-to Party
		const inputShipToParty = await frameSalesDocument.$('[title="Ship-to party"]')
		await inputShipToParty.type('1000000')
		await browser.sendKeys(Key.TAB)

		//Customer Reference
		const inputCustomerRef = await frameSalesDocument.$('[title="Customer Reference"]')
	...
	step('SAP Fiori: Save Sales Order', async (browser) => {
		...
		const SalesOrderText = await (await objMsgBar.getProperty('textContent')).jsonValue()
