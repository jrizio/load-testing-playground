...
//USAGE
//element run chrome-example.ts --no-headless --executable-path "C:\Program Files (x86)\Microsoft\Edge\Application\msedge.exe"

/**
 * by.js example
 * Makes changes to a table using clientside js and then return the updated cell valuevalue using javascript
 * Version: 1.0
 */
...
export default () => {
  ...
  step('Test: Do some sort action with tablesorter', async (browser) => {
    ...
    const firstActionEdit = await rows[0].findElement(
      By.partialLinkText('edit'),
    )
    const firstActionDelete = await rows[0].findElement(
    ...
    await browser.wait(
      Until.elementTextDoesNotMatch(newFirstCell, new RegExp(lastName)),
    )
    ...
    assert(
      ...
      'The new last name must be different from the old one.',
    )
  })
}
