...
export const settings: TestSettings = {
  loopCount: 1,
  description: 'Angular File Upload - Demo',
  actionDelay: 2.5,
  stepDelay: 2.5,
  clearCache: true,
  disableCache: true,
...
export default () => {
  ...
  step('Angular File Upload - enter username', async (browser) => {
    //Enter the username
    let input_username = By.xpath("//input[contains(@name, 'userName')]")
    let element1 = await browser.findElement(input_username)
    await element1.type('flood')
  })
